...
void RomJoints::setupMotor(int8_t id)
{
  ...
  dxl.writeControlTableItem(MAX_POSITION_LIMIT,ROM_MOTOR_IDS[id],11.3637F*(RomDefs::limits[id].max));
  dxl.writeControlTableItem(MIN_POSITION_LIMIT,ROM_MOTOR_IDS[id],11.3637F*(RomDefs::limits[id].min));
    ...
    dxl.writeControlTableItem(PROFILE_ACCELERATION, ROM_MOTOR_IDS[id], RomDefs::acc_profile[id]);    
    dxl.writeControlTableItem(PROFILE_VELOCITY, ROM_MOTOR_IDS[id], RomDefs::vel_profile[id]);
  ...
  if(recv_cnt  == DXL_ID_CNT){ 
    for(i=0; i<recv_cnt; i++){
        ...
        if((id>=ROM_MOTOR_IDS[0]) && (id<=ROM_MOTOR_IDS[DXL_ID_CNT - 1])) {
          ...
          motors[identifier].position=0.088*(sr_load_vel_pos[i].present_position) - RomDefs::offset[i]; //grad
