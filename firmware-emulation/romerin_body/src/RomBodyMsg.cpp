...
RomerinMsg configuration_message(){
    RomerinMsg msg; 
        uchar_t *punt=msg.info;
    msg.id = ROM_CONFIG;
    for(int i=0;i<4;i++)punt+=romerin_writeUChar(punt, RomBodyDefs::IP_ADDRESS[i]);
    for(int i=0;i<4;i++)punt+=romerin_writeUChar(punt, RomBodyDefs::GATEWAY_ADDRESS[i]);
    for(int i=0;i<4;i++)punt+=romerin_writeUChar(punt, RomBodyDefs::SUBNET_MASK[i]);

    punt+=romerin_writeString(punt,RomBodyDefs::MODULE_NAME);
    punt+=romerin_writeString(punt,RomBodyDefs::WIFI_SSID);
    punt+=romerin_writeString(punt,RomBodyDefs::WIFI_KEY);
...
{
    for(int i=0;i<4;i++)RomBodyDefs::IP_ADDRESS[i]=*(buffer++);
    for(int i=0;i<4;i++)RomBodyDefs::GATEWAY_ADDRESS[i]=*(buffer++);
    for(int i=0;i<4;i++)RomBodyDefs::SUBNET_MASK[i]=*(buffer++);
    buffer=romerin_getString(buffer,RomBodyDefs::MODULE_NAME,99);
    buffer=romerin_getString(buffer,RomBodyDefs::WIFI_SSID,49);
    buffer=romerin_getString(buffer,RomBodyDefs::WIFI_KEY,49); 
