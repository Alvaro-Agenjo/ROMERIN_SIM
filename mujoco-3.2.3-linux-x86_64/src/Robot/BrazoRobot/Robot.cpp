...
int main(int argc, const char** argv) {
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    control->setVentanaAbierta(true);
    control->enMovimiento(upPressed);
    if(/*control->getMoviendo() && */!control->hilosCreados){
      control->iniciarHilosMovimiento(control);
      seHaMovido = true;
      hilosCerrados = false;
    }
    if(!control->getMoviendo() && seHaMovido && !hilosCerrados && control->hilosCreados){
      control->pararHilosMovimiento();
      hilosCerrados = true;
    }
