...
void _RomKin<REAL>::FKfast(const REAL* q, REAL m[][3], REAL p[])
{
    ...
    REAL q2 = q[1] - alpha,    q3 = q[2] - alpha;
    REAL c1 = cos(q[0]), s1 = sin(q[0]), c2 = cos(q2), s2 = sin(q2);
    REAL s3_2 = sin(q3-q2), c2_3 = cos(q2 - q3);
    REAL c4 = cos(q[3]), s4 = sin(q[3]), c5 = cos(q[4]), s5 = sin(q[4]), c6 = cos(q[5]), s6 = sin(q[5]);
...
bool _RomKin<REAL>::IKwrist(REAL* q, REAL x, REAL y, REAL z, bool elbow)
{
    ...
    REAL num =x * x + y * y + z * z - Lc * Lc - Ld * Ld; 
    const REAL den = 2 * Lc * Ld;
...
void _RomKin<REAL>::q2m(REAL m[], REAL q[], bool gdl3)
{
    ...
    m[4] = 180 + rad2deg * (q[4] - q[5] / factor_6 );
    m[5] = 180 - rad2deg * (q[4] + q[5] / factor_6);
}
...
void _RomKin<REAL>::m2q(REAL q[], REAL m[], bool gdl3) {
    ...
    q[4] = (m[4] - m[5]) * deg2rad / 2;
    q[5] = (180 - (m[4] + m[5]) / 2) * deg2rad * factor_6;
}
...
void _RomKin<REAL>::mt2qt(REAL qt[], REAL mt[], bool gdl3){
    ...
    qt[4] = (mt[4] - mt[5]) * 0.00135;
    qt[5] = -(mt[4] + mt[5])  * factor_6 * 0.00135;
}
