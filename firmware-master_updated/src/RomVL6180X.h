 ...
 * 
 * Code Bases on but adapted to Romerin 
 * SparkFun_VL6180X.h
...
public:	
  ...
  VL6180x(uint8_t address=DEFAULT_VL6180X_ADDRESS);
  //changes the default address to the new one
  void setCurrentSensor(uint8_t new_address);
  ...
  uint8_t getDistance();
  void initDistanceMeasure();
  uint8_t getDistanceMeasure();
  ...
  //Change the default address of the current device and sets the
  //current sensor to that one 
 void changeAddress(uint8_t new_address);
 
  void setGPIO1(bool active=true);
...
class RomProximityArray
{
  ...
  VL6180x com_sensor3;
  uint8_t distances[3];
  filter<uint8_t,3, uint16_t> dfilter[3];
  public:
   void setup();
   void loop();
